...
// generate order by cash on delivery
const generateCashOnDeliveryOrder = asyncHandler(async (req, res) => {
  try {
    const { addressId } = req.body;
      ...
      return res
        ...
        .json(
          new ApiResponse(
            201,
            unpaidOrder,
            "Cash on delivery order generated successfully"
          )
        );
    } else {
      return res
        ...
        .json(
          new ApiResponse(
            500,
            null,
            "Something went wrong while initialising the cash on delivery order."
          )
        );
    }
  } catch (error) {
    console.log("Error in generating cash on delivery order");
...
// update order status , paymentId and isPaymentDone flag for cash on delivery orders
const updateCashOnDeliveryOrderStatus = asyncHandler(async (req, res) => {
  try {
    const { orderId } = req.params;
  ...
  */
    ...
    console.log("Error in updating cash on delivery order");
    console.log("ERROR: ", error);
    res.status(500).json({
      success: false,
      message: `Internal Server Error-${error}`,
